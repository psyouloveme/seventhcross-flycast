...
grid.PART_TYPE_HEAD = 0
grid.PART_TYPE_BODY = 1
grid.PART_TYPE_ARM  = 2
grid.PART_TYPE_LEG  = 3
...
function grid.find_part(part_type, part_level, attempts)
    ...
    for y = 0 + part_y, part_y, 1 do
        ...
        for x = 0 + part_x, 10  + part_x, 1 do
            local i = (y * 20) + x
            if (grid.get_part_level(i) == part_level) then
                table.insert(candidates, i)
                print(string.format("Candidate: index: %d at supergrid (%d,%d)", grid.coords_to_index(x, y), x, y))
            end
        end
    end
    ...
    for k,v in pairs(candidates) do
        ...
        grid.find_simplest(v, attempts)
    end
end

function grid.calculate_score(index, coords)
    print(string.format("Calculate Score: index: %d coords: ", index)..table_to_string(coords))
    local grid_ptr_cpy = GRID_DEF + (index * 100 * 4)
    local score = 0
    for i = 0, 100, 1 do
        if grid.is_index_in_list(i, coords) then
            local w = 1.0 - memory.readFloat32(grid_ptr_cpy + i)
            local wd = w
            wd = wd * wd
            local scored = score
            scored = scored + wd
            ...
            local w = memory.readFloat32(grid_ptr_cpy + i)
            local wd = w
            wd = wd * wd
            local scored = score
            scored = scored + wd
...
function grid.calculate_part(coords)
    print(string.format("Calculate Part: coords: ", coords))
    local best_idx = 0
    local best_score = 400.0
    for i = 0, 400, 1 do
        local score = grid.calculate_score(i, coords)
...
function grid.coords_to_index(x, y)
...
function grid.index_to_coords(index)
...
function grid.get_part_level(index)
...
function grid.get_part_type(index)
    ...
    if y < 10 then
        if x < 10 then
            result = grid.PART_TYPE_HEAD
        else
            result = grid.PART_TYPE_BODY
        end
    else
        if x < 10 then
            result = grid.PART_TYPE_ARM
        else
            result = grid.PART_TYPE_LEG
        end
    end
...
function grid.is_index_in_list(index, coords)
...
function grid.find_simplest(search_idx, attempts)
    ...
    while found ~= true and #pattern < attempts do
        ...
        for i = 0, 100, 1 do
            ...
            local search_score = grid.calculate_score(search_idx, pattern)
            local best_idx = 0
            local best_score = 400.0
            -- loop over checking a supergrid idx
            for j = 0, 400, 1 do
                if j ~= search_idx then
                    local score = grid.calculate_score(j, pattern)
                    if score < best_score then
                        best_idx = j
                        best_score = score
                    end
...
function grid.do_function_t(coords_list)
    ...
    local index, score = table.unpack(grid.calculate_part(indices))
    local ptype = grid.get_part_type(index)
    local plevel = grid.get_part_level(index)

    print(string.format("Found: idx: %d type: %d level: %d score: %f"), index, ptype, plevel, score)
end

-- example coords_list = {1, 1, 2, 1}
function grid.do_function_s(index, coords_list)
